...
void main() {
  ...
  test('initial state should be [VideoInitial]', () async {
    expect(videoCubit.state, VideoInitial());
  });
  ...
  group('getVideos', () {
    blocTest<VideoCubit, VideoState>(
      'emits [LoadingVideo,VideoLoaded] when getVideos is called',
      build: () {
        when(
          () => getVideos(any()),
    ...
    blocTest<VideoCubit, VideoState>(
      'emits [LoadingVideos,VideoError] when addVideo is called and fails',
      build: () {
        when(
          () => getVideos(any()),
        ).thenAnswer((_) async =>
